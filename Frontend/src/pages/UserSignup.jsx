...
const UserSignup = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post(`${import.meta.env.VITE_BASE_URL}/user/signup`, newUser);
      if (res.status === 201) {
        const data = res.data;
        setUser(data.user);
    ...
    <div className="relative h-screen w-screen">
      ...
      <div className="absolute inset-0 flex items-center justify-center">
        <div className="bg-white bg-opacity-80 p-8 rounded-lg shadow-lg w-[90%] max-w-md">
          <h2 className="text-2xl font-bold text-center text-gray-800 mb-6">Create a User Account</h2>
